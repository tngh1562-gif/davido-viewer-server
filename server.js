...
const SHOP_DEFAULTS = [
  { id: 'priority', name: '선참권',          desc: '다음 내전 팀 배치 선택 우선권', price:  300, stock: -1, icon: '⭐', rarity: 'common'    },
  { id: 'no_ban',   name: '노밴권',          desc: '다음 내전 밴 페이즈 면제',       price:  700, stock: -1, icon: '🛡️', rarity: 'uncommon'  },
  { id: 'wr_reset', name: '승률 초기화권',   desc: '내전 승률 통계 초기화',          price: 1500, stock: -1, icon: '🔄', rarity: 'rare'      },
  { id: 'extend',   name: '내전 1판 연장권', desc: '내전 1판 추가 연장',             price: 2500, stock: -1, icon: '⚡', rarity: 'epic'      },
  { id: 'all_day',  name: '종일권',          desc: '당일 모든 내전 참가 가능',       price: 8000, stock: -1, icon: '🌙', rarity: 'legendary' },
];
...
app.post('/api/game/bridge/start', async (req, res) => {
  ...
  try {
    const dr = await postJson(`${INHOUSE_SERVER_URL}/api/viewer-deduct`,
      { nickname: name, amount: bet, reason: `🪟 유리다리 베팅 (-${bet}P)` },
      { 'x-viewer-secret': VIEWER_SERVER_SECRET || 'davido-admin' });
    if (!dr.ok) { recoverFatigue(name, BRIDGE_FATIGUE); return res.json({ ok: false, error: dr.error || '포인트 부족' }); }

    // 각 행의 정답 사전 생성 (L or R)
    const answers = Array.from({length: BRIDGE_ROWS}, () => Math.random() < 0.5 ? 'L' : 'R');
    const gameId  = crypto.randomBytes(8).toString('hex');
    bridgeSessions.set(gameId, { name, bet, answers, row: 0, createdAt: Date.now() });

    const { viewers: bvs, viewer: bv } = getViewer(name);
    bv.points = dr.points; saveViewer(bvs);

    res.json({ ok: true, gameId, rows: BRIDGE_ROWS, mults: BRIDGE_MULTS, points: dr.points, fatigue: fg.fatigue });
  } catch(e) { recoverFatigue(name, BRIDGE_FATIGUE); res.json({ ok: false, error: e.message }); }
});

app.post('/api/game/bridge/step', async (req, res) => {
  const name = getSessionName(req);
  if (!name) return res.json({ ok: false, error: '로그인 필요' });
  const { gameId, choice } = req.body;
  const s = bridgeSessions.get(gameId);
  if (!s || s.name !== name) return res.json({ ok: false, error: '세션 없음' });
  if (s.row >= BRIDGE_ROWS) return res.json({ ok: false, error: '이미 완료됨' });
  if (!['L','R'].includes(choice)) return res.json({ ok: false, error: '잘못된 선택' });

  const correct = s.answers[s.row];
  const survived = choice === correct;

  if (!survived) {
    bridgeSessions.delete(gameId);
    addFeed('bridge', name, { reason: `🪟 유리다리 ${s.row}번째 칸에서 추락! (-${s.bet}P)` });
    return res.json({ ok: true, survived: false, correctWas: correct, row: s.row });
  }

  s.row++;
  const mult = BRIDGE_MULTS[s.row - 1];

  // 마지막 칸 통과 → 자동 완료
  if (s.row === BRIDGE_ROWS) {
    bridgeSessions.delete(gameId);
    const gain = Math.floor(s.bet * mult);
    try {
      const gr = await postJson(`${INHOUSE_SERVER_URL}/api/viewer-grant`,
        { nickname: name, amount: gain, reason: `🪟 유리다리 완주! (+${gain}P)` },
        { 'x-viewer-secret': VIEWER_SERVER_SECRET || 'davido-admin' });
      addFeed('bridge', name, { mult, gain, bet: s.bet, reason: `🪟 유리다리 ${mult}x 완주! (+${gain}P)` });
      return res.json({ ok: true, survived: true, completed: true, mult, gain, points: gr.points });
    } catch(e) { return res.json({ ok: false, error: e.message }); }
  }

  res.json({ ok: true, survived: true, row: s.row, mult, nextMult: BRIDGE_MULTS[s.row] });
});

app.post('/api/game/bridge/cashout', async (req, res) => {
  const name = getSessionName(req);
  if (!name) return res.json({ ok: false, error: '로그인 필요' });
  const { gameId } = req.body;
  const s = bridgeSessions.get(gameId);
  if (!s || s.name !== name) return res.json({ ok: false, error: '세션 없음' });
  if (s.row === 0) return res.json({ ok: false, error: '최소 1칸 통과 후 현금화 가능' });
  ...
  try {
    const gr = await postJson(`${INHOUSE_SERVER_URL}/api/viewer-grant`,
      { nickname: name, amount: gain, reason: `🪟 유리다리 ${mult}x 현금화 (+${gain}P)` },
      { 'x-viewer-secret': VIEWER_SERVER_SECRET || 'davido-admin' });
    addFeed('bridge', name, { mult, gain, bet: s.bet, reason: `🪟 유리다리 ${s.row}칸 ${mult}x (+${gain}P)` });
    res.json({ ok: true, mult, gain, points: gr.points });
  } catch(e) { res.json({ ok: false, error: e.message }); }
});
...
app.post('/api/game/balloon/start', async (req, res) => {
  ...
  try {
    const dr = await postJson(`${INHOUSE_SERVER_URL}/api/viewer-deduct`,
      { nickname: name, amount: bet, reason: `🎈 풍선게임 베팅 (-${bet}P)` },
      { 'x-viewer-secret': VIEWER_SERVER_SECRET || 'davido-admin' });
    if (!dr.ok) { recoverFatigue(name, BALLOON_FATIGUE); return res.json({ ok: false, error: dr.error || '포인트 부족' }); }

    const gameId = crypto.randomBytes(8).toString('hex');
    const popAt  = genBalloonPop();
    balloonSessions.set(gameId, { name, popAt, bet, createdAt: Date.now(), totalHeldMs: 0 });

    // 통계
    const { viewers: bvs, viewer: bv } = getViewer(name);
    bv.stats = bv.stats || {}; bv.stats.balloonPlayed = (bv.stats.balloonPlayed||0) + 1;
    bv.points = dr.points; saveViewer(bvs);

    res.json({ ok: true, gameId, points: dr.points, fatigue: fg.fatigue });
...
// 현금화
app.post('/api/game/balloon/cashout', async (req, res) => {
  const name = getSessionName(req);
  if (!name) return res.json({ ok: false, error: '로그인 필요' });
  const { gameId, mult } = req.body;
  const s = balloonSessions.get(gameId);
  if (!s || s.name !== name) return res.json({ ok: false, error: '세션 없음 또는 만료' });

  const m = Math.max(1, parseFloat(mult) || 1);
  if (m >= s.popAt) {
    balloonSessions.delete(gameId);
    return res.json({ ok: false, error: '이미 터진 풍선입니다', popped: true, popAt: s.popAt });
  }
